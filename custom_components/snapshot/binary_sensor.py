...
class AvailabilitySensor(CoordinatorEntity, BinarySensorEntity):
    ...
    def __init__(self, coordinator, entry):
        ...
        self._attr_name = f"{entry.title} Snaphot Availability"
        
    @property
    def name(self):
        return f"{ self._entry.title } Snaphot Availability"
        
    @property
    def device_info(self):
        return {
            "identifiers": {(DOMAIN, self._entry.entry_id)},
            "name": f"{ self._entry.title } Snaphot",
            "manufacturer": "Snapshot",
        }
    
    async def async_update(self):
        _LOGGER.error(f"Update { self.platform.config_entry.options.get('url','') }")
